...
def new_main(gametext):
    pgnfile = io.StringIO(gametext)

    analyzed_games = ""

    engine = "./bin/stockfish".split()

    try:
        # with open(pgnfile) as pgn:
        for game in iter(lambda: chess.pgn.read_game(pgnfile), None):
            try:
                ...
                analyzed_games += str(analyzed_game)
                analyzed_games += '\n\n'
...
def one_game(gametext, gametime):
    if sys.platform == "darwin":
        engine = "./bin/stockfish-mac".split()
    else:
        engine = "./bin/stockfish".split()
    pgnfile = io.StringIO(gametext)
    game = chess.pgn.read_game(pgnfile)
    analysis = analyze_game(game, gametime, engine, "1")
